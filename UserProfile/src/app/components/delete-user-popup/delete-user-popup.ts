...
export class DeleteUserPopup {
  ...
  async deleteUser() {
    try {
      const response: IUser = await this.clientHttp.deleteUser(this.user_id()); // Nota: Cambiar this.user_id para simular error
      ...
      await this.router.navigate(['/**']); // No se si es una forma muy burda de refrescar la página, pero es la forma más sencilla que he encontrado sin salirme del "scope" de lo que se enseña en clase. Me he encontrado que cuando borro un usuario, al estar en la misma página, el componente no se refresca y entonces no se ejecuta la petición para obtener todos los usuarios "nuevos actualizados".
